...
# 5. Table 4.1 and Figures 4.1-4.2: OLS feature-block model fit
  ...
  if (!"adjusted_r_squared" %in% names(main_fit) && "adj.r.squared" %in% names(main_fit)) {
    main_fit <- main_fit %>%
      mutate(adjusted_r_squared = as.numeric(.data[["adj.r.squared"]]))
  }
...
main_fit_clean <- main_fit %>%
  ...
  mutate(
    ...
    adjusted_r_squared = as.numeric(adjusted_r_squared)
  ) %>%
...
# 9. Appendix Table A.6: Alternative model fit summary
