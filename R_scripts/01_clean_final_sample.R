...
#
# MSc thesis data pipeline: Billboard trajectories, Spotify audio,
# and lyrics features.
#
# Purpose:
#   Build the song-level final sample used in the thesis models.
#   The code keeps the original matching and feature-engineering logic,
#   but writes the final modeling dataset under the clearer name
...
# Reviewed supplemental matches.
# Spotify supplemental matches were not automatically accepted in the working pipeline.
# Lyric supplemental Top 10 matches were later accepted after review in the messy script.
...
# ============================================================
# The thesis combines weekly Billboard chart data, Spotify audio features,
# and lyric data. We import them explicitly here so the pipeline can run
# from a fresh R session without relying on preloaded objects.
...
# ============================================================
# These variables use only the internal Billboard song universe.
...
# Matching proceeds from strict exact matches to conservative fuzzy matches, preserving match tiers for robustness checks.
# ============================================================
...
# Lyrics are cleaned and deduplicated separately because lyric sources use different title and artist conventions than Billboard.
# ============================================================
# billboard_lyrics_spotify is used only for lyrics and lyric-derived variables.
# Audio columns from this source are deliberately not used.
...
# Keep the thesis modeling variables plus the minimum additions needed for robustness checks and defense.
...
# Do not impute rise_speed or decay_rate.
# Do not replace structural missing values with zero.
...
vif_engineered_legacy <- tibble(
  note = "VIF deferred to modeling script; no lm() calls are made in this feature-engineering script."
)
